...
def main(image_path, model_path, tile_width, tile_height, combine_results):
    if tile_width == 0:
        tile_width = None
    if tile_height == 0:
        tile_height = None
...
def poly_approx(image, name):
    ...
    with open('final_data/' + name + '_holds.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('image_file', type=str, default='', help='The input wall image file')
    ...
    main(args.image_file, args.model_path, args.tile_width, args.tile_height, args.combine_results)
